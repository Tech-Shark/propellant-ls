...
export default function CredentialVerification() {
  ...
  const fetchAllCredentials = async () => {
    ...
    try {
      ...
      // Log verified credentials response for debugging
      console.log("Verified credentials API response:", verifiedRes.data);
      if (verifiedRes.data?.data?.data) {
        verifiedRes.data.data.data.forEach((cred, idx) => {
  ...
  const handleApprove = async (credentialId: string) => {
    try {
      const response = await axiosInstance.patch(
        `/credentials/${credentialId}/approve-coredao`,
        {} // send empty object so backend can auto-fill metadataUri
      );
      toast({
